...
object `package` {
  ...
  @implicitNotFound(msg = "Cannot find scalaxb.XMLFormat type class for ${A}")
  ...
  @implicitNotFound(msg = "Cannot find scalaxb.CanWriteXML type class for ${A}")
...
object DataRecord extends XMLStandardTypes {
  ...
  // this is for choice option: scalaxb.DataRecord(x.namespace, Some(x.name), fromXML[Address](x))
  ...
  def toXML[A](obj: DataRecord[A], namespace: Option[String], elementLabel: Option[String],
    ...
    case _ => sys.error("unknown scalaxb.DataRecord.")
  }
}
...
trait AnyElemNameParser extends scala.util.parsing.combinator.Parsers {
  ...
  // we need this so treat scalaxb.ElemName as NodeSeq for fromXML etc.
